...
def _answer_to_bags(answer: Union[str, List[str], Tuple[str, ...]], language="en") -> Tuple[List[str], List[Set[str]]]:
...
def get_metrics(predicted: Union[str, List[str], Tuple[str, ...]],
                gold: Union[str, List[str], Tuple[str, ...]], language="en") -> Tuple[float, float]:
    """
...
def metric_max_over_ground_truths(metric_fn, predictions, ground_truths, language="en"):
...
def get_answer_str(answers: list, scale: str):
    """
    :param answers: List of answers, could be numeric or non-numeric strings
    :param scale: Scale type ("", "thousand", "million", "billion", "percent")
    :return: Formatted answer strings
    """
    def safe_to_float(value):
        """Safely convert a number to float, or return None if out of range."""
        try:
            return float(value)
    ...
    for ans in sorted_ans:
        ans_str = str(ans)
        if is_number(ans_str):
            ans_num = to_number(ans_str)  # Assumes `to_number` converts string to number
            if ans_num is None:
                if scale:
                    ans_str = ans_str + " " + str(scale)
            else:
                if '%' in ans_str:  # Answer is already a percentage
                    ans_str = '%.4f' % ans_num
                else:
                    # Safely handle scale multiplication
                    try:
                        ans_str = '%.4f' % (round(ans_num, 2) * scale_to_num(scale))
                    except (OverflowError, ValueError):
                        ans_str = f"0"
